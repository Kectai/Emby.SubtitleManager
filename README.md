...
- 媒体库浏览：在插件页面按层级浏览电影、剧集、季、文件夹和 extras 视频。
- 媒体搜索：按名称查找媒体项，并以树形结果展示匹配内容。
...
- 界面语言：插件页面和接口提示跟随 Emby 首选显示语言，支持简体中文、繁体中文、繁体中文（香港）、English (United Kingdom)、English (United States)、日本語和한국어；其他语言回退为 English (United States)。
...
手动安装 Emby 插件时，DLL 应放在 Emby Server Data Folder 下的 `plugins` 子目录。可在 Emby Server Dashboard 的 Server Info 中查看当前数据目录。
...
- 删除接口只允许删除当前媒体项已识别的外部字幕，并且路径必须位于该媒体的 Emby 元数据目录内。
- 后端接口仅允许 Emby 管理员账号访问。
- 插件面向个人或可信管理员环境使用，请不要把 Emby 管理端暴露给不可信网络。

## API

这些接口主要供插件前端页面调用：

- `GET /SubtitleManager/Libraries`：获取媒体库列表。
- `GET /SubtitleManager/Items`：获取媒体项列表，默认不携带字幕流；可用 `IncludeSubtitles=true` 请求字幕信息。
- `GET /SubtitleManager/Localization`：获取插件页面使用的语言。
- `POST /SubtitleManager/Upload`：上传字幕文件，参数包含 `ItemId`、`Language`、`Format`、`IsForced` 和字幕文件。
- `POST /SubtitleManager/DeleteSubtitle`：删除当前媒体项元数据目录中的外部字幕，参数包含 `ItemId` 和 `SubtitlePath`。
